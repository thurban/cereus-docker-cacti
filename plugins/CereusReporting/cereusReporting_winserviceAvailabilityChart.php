<?php
	...
	File:         $Id: cereusReporting_winserviceAvailabilityChart.php,v 412969a4f743 2020/12/10 07:06:31 thurban $
	...
	$textBoxObj = $c->addTitle( "Win Service Availability Chart", readConfigOption( 'nmid_avail_font' ), 15 );
	...
	{
		CereusReporting_logger( "Adding tree [$tree_id] to chart", 'debug', 'availabilityWinService' );
	...
	{
		CereusReporting_logger( "Adding leaf [$leafid] from tree [$tree_id] to chart", 'debug', 'availabilityWinService' );
		...
		while ( $row = mysql_fetch_assoc( $result ) ) {
			if ( $row[ 'host_id' ] > 0 ) {
				CereusReporting_logger( "Adding host [" . $row[ 'host_id' ] . "] to chart", 'debug', 'availabilityWinService' );
				addServerToChart( $row[ 'host_id' ] );
			}
		}
	}
    ...
    ";
		...
		if ( $serviceResult ) {
			while ( $a_hostServiceState = mysql_fetch_assoc( $serviceResult ) ) {
				if ( CereusReporting_getFailedPollInSLA( $a_hostServiceState[ 'timeStamp' ], $slaTimeFrame, 1 ) > 0 ) {
					$taskNo[ ]    = $serverCount;
					$startDate[ ] = chartTime2( $a_hostServiceState[ 'timeStamp' ] );
					$endDate[ ]   = chartTime2( $a_hostServiceState[ 'timeStamp' ] + CereusReporting_getGraphPollerInterval( $ldid ) );
					$colors[ ]    = 0xcc0000;
				}
			}
		...
		$hostSLATimeFrame   = db_fetch_cell( 'select nmid_host_sla_timeframe from host where id=' . $deviceId );
		...
		$totalPolls = CereusReporting_getGraphTotalPolls( $ldid, $startTime, $endTime, $slaTimeFrame );
	...
	function addServerToChart( $deviceId )
	{
		global $dataMode, $labels, $taskNo, $startDate, $endDate, $colors, $serverCount, $startTime, $endTime, $globalSLA,
		       $startDateInSLA, $startDateOutSLA, $endDateInSLA, $endDateOutSLA, $outSLAColorArray, $inSLAColorArray,
		       $outTaskNo, $inTaskNo, $endTimeSpan, $deviceToTask, $slaTime_id, $modeTimeFrame, $outageEx;
