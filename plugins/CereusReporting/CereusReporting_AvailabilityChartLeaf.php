<?php
    ...
    while ( $row = $stmt->fetch() ) {
        ...
        ";
        $stmt_slaTimes = $db->prepare( $s_timeframes_sql);
        $stmt_slaTimes->execute();
        while ($tfRow = $stmt_slaTimes->fetch() ) {
    ...
    $c->addTitle("Availability Report - ".$hostDescription.' [ '.$hostIp.' ]', "timesbi.ttf", 14);
