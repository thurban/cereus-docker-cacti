<?php
	/*******************************************************************************
	 *
	 * File:         $Id: createPDFReport_scheduler.php,v 40a17197e8c9 2017/07/18 06:44:34 thurban $
...
/* we are not talking to the browser */
	...
	$archiveReport = getPreparedDBValue( 'SELECT archiveReport FROM plugin_nmidCreatePDF_Reports_scheduler WHERE ScheduleId=?;',array($scheduleId));
	$archiveUserGroup = getPreparedDBValue( 'SELECT archiveUserGroupId FROM plugin_nmidCreatePDF_Reports_scheduler WHERE ScheduleId=?;',array($scheduleId));
	...
	CereusReporting_logger( 'Creating scheduled Report [' . $reportTitle . ']', "notice", "PDFCreation" );
	...
	$reportOutputFile = '';
	if ( $outputType == 1 ) {
		$reportOutputFile = sys_get_temp_dir() . DIRECTORY_SEPARATOR . $title . '_' . date( "Ymd-Hi", $startTime ) . '-' . date( "Ymd-Hi", $endTime ) . '.html';
	}
	else {
		$reportOutputFile = sys_get_temp_dir() . DIRECTORY_SEPARATOR . $title . '_' . date( "Ymd-Hi", $startTime ) . '-' . date( "Ymd-Hi", $endTime ) . '.pdf';
	}
...
// Cleanup files
	...
	CereusReporting_logger( 'Saving the PDF Output to disk [' . $reportOutputFile . '].', "debug", "PDFCreation" );
	$report->Output( $reportOutputFile, "F" );

	if ( $outputType == 1 ) {
		$reportOutputFile = $reportOutputFile . '.zip';
	}
	...
	CereusReporting_logger( 'Returning the full filename[' . $reportOutputFile . '].', "debug", "PDFCreation" );
	`echo "$reportOutputFile" > $fileDat`;
