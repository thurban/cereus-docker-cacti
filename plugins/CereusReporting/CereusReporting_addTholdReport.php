<?php
	/*******************************************************************************
	 *
	 * File:         $Id: CereusReporting_addTholdReport.php,v 40a17197e8c9 2017/07/18 06:44:34 thurban $
	...
	function form_display( $reportId )
	{
		...
		FROM 
			thold_data 
		INNER JOIN host ON host.id = thold_data.host_id
		" . $where_clause
		);

		html_start_box( "<strong>Thold Settings</strong>", "100%", $colors[ "header" ], "3", "center", "" );

		form_hidden_box( "TholdId", $reportId, "" );
		form_hidden_box( "save_component_import", "1", "" );
		form_hidden_box( "itemType", "tholddata", "" );
		...
		if ( sizeof( $a_tholdData ) > 0 ) {
			...
			foreach ( $a_tholdData as $s_tholdData ) {
				...
				form_selectable_cell( $s_tholdData[ 'name' ] . ' [' . $s_tholdData[ 'description' ] . ']', $s_tholdData[ 'id' ] );
				form_selectable_cell( "<a href='CereusReporting_addMultiReport.php?action=save&save_component_import=1&itemType=12&Data=" . $s_tholdData[ 'id' ] . "&ReportId=" . $reportId . "'><b>Add to Report</b></a>", $s_tholdData[ 'id' ], 250 );
				form_end_row();
			}
			html_end_box( FALSE );
		}
		else {
			print "<tr><td><em>No Tholds exist</em></td></tr>";
			html_end_box( FALSE );
		}
